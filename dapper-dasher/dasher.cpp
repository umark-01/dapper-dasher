...
{   // window dimensions
    ...
    InitWindow(windowDimensions[0], windowDimensions[1], "Window");
    
    int velocity{0};
    int nebulaVel{-200}; // pixel / s
    const int  jumpVel{-600}; // pixel/sec
    const int gravity{1'000}; // acceleration (pixel / s)/ s
    bool isInAir{};
    bool collusion{};
    ...
    {   
        ...
        // backrground scroll pos
        bgX -= 20 * deltaTime;
        if (bgX <= -background.width * 2)
        {
            bgX = 0.0;
        }
        // midg scroll animation
        mgX -= 40 * deltaTime;
        if(mgX <= - midground.width * 2)
        {
            mgX = 0.0;
        }
        // foreg scroll animation
        fgX -= 80 * deltaTime;
